...
public class RPC extends Module {
    ...
    private final Setting<String> title = sgGeneral.add(new StringSetting.Builder().name("title").description("What to display as the RPC's title.").defaultValue("Notware {oversion} Re-coded by NotMe11").onChanged(booleanSetting -> alertTitle()).build());
    ...
    public RPC() {
        super(NotWare.nwmisc, "RPC", "Notware RPC for Discord!");
    }
    ...
    public void onActivate() {
        ...
        if (messages.get().isEmpty() || messages2.get().isEmpty()) {
            error( "Monke your RPC messages are empty!");
            this.toggle();
            return;
        }
        updateDelay = delay.get() * 20;
        DiscordEventHandlers handlers = new DiscordEventHandlers();
        instance.Discord_Initialize("862373776791830548", handlers, true, null);
