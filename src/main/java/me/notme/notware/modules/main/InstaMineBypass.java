...
public class InstaMineBypass extends Module {
    ...
    private final Setting<Boolean> pick = sgGeneral.add(new BoolSetting.Builder().name("only-pick").description("Only tries to mine the block if you are holding a pickaxe.").defaultValue(true).build());
    private final Setting<Boolean> ironPick = sgGeneral.add(new BoolSetting.Builder().name("iron-pick").description("Allows iron pickaxe to count as a pickaxe for instamine.").defaultValue(false).visible(pick::get).build());
    ...
    private final Setting<Integer> bypassDelay = sgDelay.add(new IntSetting.Builder().name("bypass delay").description("The delay between breaks for bypass mode.").defaultValue(4).min(1).sliderMax(100).build());
    private final Setting<Integer> bypassIterations = sgDelay.add(new IntSetting.Builder().name("bypass iterations").description("The amount of times bypass mode should happen.").defaultValue(0).min(0).sliderMax(20).build());
    private final Setting<Integer> normalDelay = sgDelay.add(new IntSetting.Builder().name("normal delay").description("The delay between breaks for normal mode.").defaultValue(41).min(1).sliderMax(100).build());
    private final Setting<Integer> normalIterations = sgDelay.add(new IntSetting.Builder().name("normal iterations").description("The amount of times bypass mode should happen.").defaultValue(1).min(0).sliderMax(20).build());
    ...
    public InstaMineBypass() {
        super(NotWare.nwcombat, "insta-mine-bypass", "sdusahdusafsahfuahguhadosjdad");
    }
