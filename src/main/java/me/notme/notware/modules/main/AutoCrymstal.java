...
public class AutoCrymstal extends Module {
    ...
    public AutoCrymstal() {
        super(NotWare.nwcombat, "auto-crymstal", ":sunglasses:");
    }
    ...
    private void onPreTick(TickEvent.Pre event) {
        ...
        {
            error("might crash u, disabling ca");
            toggle();
            return;
        ...
        if (cevPause.get() && Modules.get().isActive(AutoCrystalHead.class)) return;
        if (killAuraPause.get() && (Modules.get().isActive(KillAura.class))) return;
        if(burrowPause.get() && NotwareUtils.isBurrowedLiving(bestTarget)) return;
        if ((baPause.get() && Modules.get().isActive(BedAura.class)) || (baPause.get() && Modules.get().isActive(BedAura.class))) return;
    ...
    private boolean shouldFacePlace(BlockPos crystal) {
        for (LivingEntity target : targets) {
            ...
            if (!faceSurrounded.get() && NotwareUtils.inFace(target)) return false;
            if (surroundHoldPause.get() && surroundHold.get() && NotwareUtils.isSurroundBroken(target)) return false;

            if (crystal.getY() == pos.getY() + 1 && Math.abs(pos.getX() - crystal.getX()) <= 1 && Math.abs(pos.getZ() - crystal.getZ()) <= 1) {
                if (greenHolers.get() && NotwareUtils.isGreenHole(target)) return true;
                if (NotwareUtils.getTotalHealthLiving(target) <= facePlaceHealth.get()) return true;
    ...
    private boolean shouldSurroundBreak(BlockPos crystal) {
        for (LivingEntity target : targets) {
            ...
            if (!NotwareUtils.isBedrock(pos.north(1)) && crystal.equals(pos.north(2))) return true;
            if (!NotwareUtils.isBedrock(pos.west(1)) && crystal.equals(pos.west(2))) return true;
            if (!NotwareUtils.isBedrock(pos.south(1)) && crystal.equals(pos.south(2))) return true;
            if (!NotwareUtils.isBedrock(pos.east(1)) && crystal.equals(pos.east(2))) return true;
        }
        return false;
    }
    ...
    private boolean isSurroundBreaking() {
        if (surroundBreak.get() && bestTarget != null) {
            ...
            if (!NotwareUtils.isSurrounded(bestTarget)) return false;
            if (NotwareUtils.isGreenHole(bestTarget)) return false;
            return true;
        }
    ...
    private boolean isSurroundHolding() {
        if (surroundHold.get() && bestTarget != null && NotwareUtils.isSurroundBroken(bestTarget)) {
            return true;
        }
